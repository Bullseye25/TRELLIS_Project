...
try:
    ...
    # Amplification factor: shorter legs need larger angle sweeps to cover visible ground step
    leg_amplification = min(1.8, max(1.0, 0.85 / leg_scale))
    
    BOB_SCALE = 1.0
    SWAY_SCALE = 1.8  # Sway amplification for chibi waddle
    ...
    for frame in range(start_frame, end_frame + 1):
        ...
        for tgt_pbone in target_arm.pose.bones:
            ...
            if bone_name in ["mixamorig:LeftUpLeg", "mixamorig:RightUpLeg"]:
                # Thigh swing is mainly Pitch (X) and Yaw (Z)
                ...
                # Hips waddle yaw (Y) and roll (Z)
                euler = rot.to_euler('XYZ')
                euler.z *= 2.0  # side-to-side sway
                euler.y *= 1.5  # twist
                rot = euler.to_quaternion()
...
def retarget_walk_anim(mesh_fbx: str, anim_fbx: str) -> bytes:
...
def main(choice: str = None, no_download: bool = False):
    print("==================================================")
    print("   Universal Chibi Walking Animation Generator    ")
    print("==================================================")
    ...
    if use_modal:
        ...
        # Scan for FBX files, excluding Walking.fbx and output walk files
        fbxs = [f for f in files if f.endswith(".fbx") and f.lower() not in ["walking.fbx", "idle.fbx"] and not f.endswith("_walk.fbx") and not f.endswith("_idle.fbx") and not f.endswith("_thinking.fbx")]
        ...
        fbxs = [f for f in os.listdir(downloads_dir) if f.endswith(".fbx") and f.lower() != "walking.fbx" and not f.endswith("_walk.fbx")]
    ...
    print("\n==================================================")
    print("   All universal chibi walk animations completed! ")
    print("==================================================")
